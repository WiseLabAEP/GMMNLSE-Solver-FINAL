...
lambda0_build = 1550e-9; % the center wavelength used to solve for the modes, in m
lambda0_disp = 1550e-9; % the center wavelength at which the dispersion parameters will be evaluated, in m
lrange = 180e-9; % wavelength range, in m. Should be the same as the range used to calculate the modes
modes_list=[1:6 15 28]; % modes selected for dispersion calculation (usually 1:num_modes, but can be different)
...
folder_name = 'Fibers/GRIN_1550'; % location to which files are saved
...
save([folder_name sep_char 'betas'], 'betas');
