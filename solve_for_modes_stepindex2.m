...
Nf = 40; % number of frequency points at which the modes will be calculated
lambda0 = 1030e-9; % center wavelength, in m
lrange = 180e-9; % wavelength range, in m. If 0 only the center wavelength will be used
num_modes = 15; % number of modes to compute
radius = 7; % outer radius of fiber, in um
folder_name = 'Fibers/STEP_1030S'; % folder where the output will be stored

Nx = 800; % number of spatial grid points
spatial_window = 100; % full spatial window size, in um
profile_function = @build_GRIN; % function that builds the fiber
extra_params.ncore_diff = 0.008; % difference between the index at the center of the core, and the cladding
extra_params.alpha = 60.08; % Shape parameter
