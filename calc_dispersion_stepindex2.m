...
Nf = 40;
lambda0_build = 1030e-9; % m, the center wavelength used to solve for the modes
lambda0_disp = 1030e-9; % m, the center wavelength at which the dispersion parameters will be evaluated
lrange = 180e-9; % wavelength range, in m. Should be the same as the range used to calculate the modes
modes_list=[1:10]; % usually just 1:num_modes, but it can be different

polynomial_fit_order = 9;
num_disp_orders = 4; % i.e. if this is 3, 4 coefficients will be calculated, including the 0th order

radius = 7; % Only needed for the file name
folder_name = 'Fibers/STEP_1030S';
