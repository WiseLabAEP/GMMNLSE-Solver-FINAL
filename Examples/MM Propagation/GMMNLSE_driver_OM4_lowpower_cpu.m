% Example propagation in GRIN fiber using CPU, at a relatively low power.
...
for ord=1:size(betas,1)
betas(ord,:)= betas(ord,:)*unit_conversion(ord);  
end
...
sim.verbose = 1; % Whether or not to print out information about the convergence and timing each step
    ...
    sim.gpu_yes = 0; % Whether or not to use the GPU. Using the GPU is HIGHLY recommended, as a speedup of 50-100x should be possible
    sim.mpa_yes = 0; % Whether to use the MPA algorithm or the split-step algorithm. If using a GPU the MPA algorithm will almost always be faster, but if using a CPU the split step algorithm will probably be faster
