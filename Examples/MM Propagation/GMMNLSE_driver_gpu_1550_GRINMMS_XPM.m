% This script simulates nonlinear propagation in GRIN fiber, including only
% the SPM and XPM terms.
...
num_modes = 8; % The number of modes to be simulated. 
...
% eliminate FWM terms from the SR tensor
for n1=1:num_modes
    for n2=1:num_modes    
        for n3=1:num_modes
         for n4=1:num_modes
...
sim.save_period = fiber.L0/500; % Length between saves, in m. Alternatively, 0 sets this to the fiber length. This must fit perfectly into the full propagation length
...
% propagation
save_name = make_test_save_name(['GRIN_1550_GRINMMS_XPM' num2str(run)], sim);
savepath = make_test_save_name([pwd '\plots\GRIN_1550_GRINMMS_XPM' num2str(run)], sim);
...
center = -15;
time_window = 50; % ps, Full time extent of the time window. This should be large enough to contain the pulse, but it will also effect the frequency domain
total_energy = 3*(run-1)+0.01; % nJ, total energy of the initial pulse. By convension this is the total energy in all modes
...
A0 = sum(abs(E_xyt).^2, 2)*dx; % Integrate over x
