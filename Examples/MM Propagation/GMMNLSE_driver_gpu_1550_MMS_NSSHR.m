...
% 1550 nm with no Raman or self-steepening, with higher resolution. 
% The original script was converted here into function form to more easily
% run multiple simulations in sequence.

function GMMNLSE_driver_gpu_1550_MMS_NSSHR(TFWHM,idx)
...
sim.deltaZ = 12.5e-6; % The step size in m. This may need to be 1-50 um to account for intermodal beating, even if the nonlinear length is much larger than 50 um
...
% propagation
save_name = make_test_save_name(['GRIN_1550_MMS_NSSHR' num2str(idx)], sim);
savepath = make_test_save_name([pwd '\plots\GRIN_1550_MMS_NSSHR' num2str(idx)], sim);
